...
const form = useForm({
    ...
    trial_days: props.data?.trial_days || "",
...
watch(
    ...
    (newData) => {
        ...
        form.interval = newData?.interval || "";
        form.trial_days = newData?.trial_days || "";
...
function submitForm() {
    if (props.type == "create") {
        ...
        router.put('plan/' + props.data.slug, form, {
            onSuccess: () => {
                form.name = "",
                form.price = "",
                form.currency = "",
...
<template>
    <form @submit.prevent="submitForm">
        ...
        <div class="grid gap-4 mb-4 sm:grid-cols-2">
            <div>
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Plan name<span
            ...
            <div>
                ...
                <input type="number" min="1" v-model="form.price"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    placeholder="categorie name" />
            ...
            <div>
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Interval</label>
                <input type="text"  v-model="form.interval"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    placeholder="Interval" />
                <InputError :message="form.errors.interval" class="mt-2" />
            </div>
            <div>
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">trial days</label>
                <input type="text"  v-model="form.trial_days"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    placeholder="Currency" />
